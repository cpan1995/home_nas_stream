...
export class VidSrcProvider extends BaseProvider {
    ...
    readonly BASE_URL = streamSetting('STREAM_CORE_VIDSRC_BASE_URL');
    readonly HEADERS = {
        'User-Agent':
        ...
        try {
            ...
            const sources: Source[] = m3u8Urls.map((url) => ({
                url: this.createProxyUrl(url, {
                    ...this.HEADERS,
                    Referer: `${streamSetting('STREAM_CORE_VIDSRC_DECRYPT_ORIGIN')}/`,
                    Origin: streamSetting('STREAM_CORE_VIDSRC_DECRYPT_ORIGIN')
                }),
                type: 'hls',
                quality: 'Auto',
                audioTracks: [
