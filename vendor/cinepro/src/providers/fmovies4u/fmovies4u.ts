...
export class Fmovies4U extends BaseProvider {
    ...
    readonly BASE_URL = streamSetting('STREAM_CORE_FMOVIES4U_BASE_URL');
    readonly HEADERS = {
        'User-Agent':
                ...
                sources.push({
                    url: this.createProxyUrl(
                        ...
                        stream.link.includes('file2')
                            ? {
                                  referer: `${streamSetting('STREAM_CORE_FMOVIES4U_PLAYER_ORIGIN')}/`,
                                  origin: streamSetting('STREAM_CORE_FMOVIES4U_PLAYER_ORIGIN')
                              }
                            : {
                                  ...this.HEADERS,
                                  Referer: this.buildRefererUrl(media)
