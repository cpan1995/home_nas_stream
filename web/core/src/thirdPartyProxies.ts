...
export const knownThirdPartyProxies: Record<string, RegExp[]> = {
    ...
    [streamSetting('STREAM_CORE_PROXY_HLS_ORIGIN')]: [/\/proxy\/(.+)$/],

    [streamSetting('STREAM_CORE_PROXY_MADPLAY_ORIGIN')]: [/\/api\/[^/]+\/proxy\?url=(.+)$/],

    [streamSetting('STREAM_CORE_PROXY_SMASHYSTREAM_ORIGIN')]: [/\/proxy\/m3u8\/(.+?)\/[^/]+$/],
