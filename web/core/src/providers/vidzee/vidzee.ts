...
export class VidZeeProvider extends BaseProvider {
    ...
    readonly BASE_URL = streamSetting('STREAM_CORE_VIDZEE_BASE_URL');
    readonly PLAYER_URL = streamSetting('STREAM_CORE_VIDZEE_PLAYER_URL');
    readonly HEADERS = {
        'User-Agent':
            ...
            const sources: Source[] = uniqueLinks.map((link) => ({
                url: this.createProxyUrl(
                    ...
                    link.includes('fast33lane')
                        ? {
                              referer: `${streamSetting('STREAM_CORE_VIDZEE_DECRYPT_ORIGIN')}/`,
                              origin: streamSetting('STREAM_CORE_VIDZEE_DECRYPT_ORIGIN')
                          }
